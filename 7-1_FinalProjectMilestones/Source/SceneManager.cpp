...
// SceneManager.cpp
// ================
// Manage the preparation and rendering of 3D scenes textures, materials and lighting
// ================
// AUTHOR: Nick Tilton
///////////////////////////////////////////////////////////////////////////////
...
void SceneManager::PrepareScene()
{
    ...
    CreateGLTexture("textures/wood.jpg", "wooden_table");
    CreateGLTexture("textures/candle_wax.jpg", "candle_wax");
    CreateGLTexture("textures/coffee_cupv3.png", "coffee_cupv3"); 
...
void SceneManager::RenderScene()
{
    ...
    // Enable for depth testing to ensure correct rendering
    ...
    // Render plane (table surface) with wood texture and lighting
    ...
    // Render mug (cylinder) with thickness and texture
    ...
    // Render pencil feature (metallic band)
    ...
    // Render pencil tip (small cone)
    scaleXYZ = glm::vec3(0.02f, 0.09f, 0.03f);
    ...
    // Render candle (cylinder) with wax texture
    scaleXYZ = glm::vec3(0.5f, 1.0f, 0.5f);
    positionXYZ = glm::vec3(2.0f, 0.1f, 6.0f); // Position where book was, adjusted
    RenderShapeWithEdges([this, scaleXYZ, positionXYZ]() {
        SetTransformations(scaleXYZ, 0.0f, 0.0f, 0.0f, positionXYZ);
    ...
    // Render candle flame (small cylinder)
    scaleXYZ = glm::vec3(0.2f, 0.3f, 0.2f);
    positionXYZ = glm::vec3(2.0f, 1.1f, 6.0f); // Above candle body
    RenderShapeWithEdges([this, scaleXYZ, positionXYZ]() {
        SetTransformations(scaleXYZ, 0.0f, 0.0f, 0.0f, positionXYZ);
        SetShaderColor(1.0f, 0.8f, 0.0f, 1.0f); // Yellow-orange flame
        m_basicMeshes->DrawCylinderMesh(true, true, true);
        }, glm::vec4(1.0f));

    // Render instrument base (cylinder)
    ...
    // Render instrument knob (cylinder)
    ...
    // Render instrument arm (cylinder) base
    ...
    // Render instrument hook to arm (cylinder)
    scaleXYZ = glm::vec3(0.5f, 1.0f, 0.5f);
    ...
    // Render instrument stand (cylinder)
    ...
    // Render instrument arm (cylinder) at top
    ...
    // Render instrument bulb hook to instrument (cylinder)
    ...
    // Render instrument head/glass (cylinder) with thickness
    ...
    // Render light bulb (sphere) as glowing light
    // Enable blending for transparency
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glDepthMask(GL_FALSE);
...
void SceneManager::RenderTransparentShape(std::function<void()> drawFunction, const glm::vec4& color)
{
    ...
    // Render wireframe edges (for additional detail)
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    SetShaderColor(1.0f, 1.0f, 1.0f, color.a * 0.7f);  
    drawFunction();
